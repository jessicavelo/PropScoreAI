...
def load_properties():
    ...
    for i, row in df.iterrows():
        ...
        price_per_sqft = (est_value / building_sqft) if (est_value and building_sqft) else None
        ...
        # Business data is already joined per-row (see build_atherton_master.py)
        # rather than needing a separate scraped-batch join like Santa Clara.
        businesses = []
        biz_name = clean(row.get("Business Name"))
        if biz_name:
            categories_text = clean(row.get("Business Category")).lower()
            businesses.append({
                "title": biz_name,
                "norm_addr": norm_addr,
                "lat": lat, "lng": lng,
                "category": clean(row.get("Business Category")),
                "categories_text": categories_text,
                "permanently_closed": bool(row.get("Business Closed")) if clean(row.get("Business Closed")) else False,
                "temporarily_closed": False,
                "reviews_count": number(row.get("Business Reviews")),
                "rating": number(row.get("Business Rating")),
            })
        ...
        record = {
            ...
            "years_held": None,  # not present in the merged master (no Last Sale Date column yet)
            "condition_text": "",
            "property_url": "",
